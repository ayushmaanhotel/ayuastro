...
export function getTopTraits(traits: TraitScore[], count: number = 3): TraitScore[] {
  return [...traits].sort((a, b) => b.score - a.score).slice(0, count);
}

export function getArchetype(traits: TraitScore[]): string {
...
export function getArchetypeEmoji(archetype: string): string {
...
export function getShareText(
  ...
  const traitStr = topTraits.map((t) => `${t.label || t.name} ${Math.round(t.score)}%`).join(', ');
  return `✦ ${name || 'Cosmic Seeker'} — ${archetype}\n\n🌟 Sun: ${sunSign} | 🌙 Moon: ${moonSign} | ⬆️ Asc: ${ascendant}\n\n💫 Top Traits: ${traitStr}\n\n✦ Generated by AyuAstro — AI-Powered Emotional Intelligence Platform`;
}

const ShareableCard = forwardRef<HTMLDivElement>(function ShareableCard(_props, ref) {
